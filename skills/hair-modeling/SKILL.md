...
**ステータス: v0（2026-08-30作成。inspect_hair.py 0.3.0。自己テスト 98 項目に加え、
Blender 4.5.13（PyPI の standalone bpy）＋合成シーンで bpy 依存経路を実測済み。
GUI Blender と実アセットでは未実測）。**
...
## 原則0: 初回実測プロトコル（状態: 部分実施 2026-08-31 / Blender 4.5.13 / PyPI の standalone bpy）

手順 1・2・4 は実施済み、手順 3 は合成カード 6 枚で実施、**手順 5 は未実施**。
結果は verified-facts.md の「原則0（初回実測プロトコル）の結果」を参照。
**ユーザーの GUI Blender と実アセットでは一度も動かしていないので v1 には上げない。**
実アバターを触るセッションで手順 3（実物での突き合わせ）と手順 5 をやり直すこと。
...
- **判定ルールの落とし穴（修正済み。同じ間違いを繰り返さないこと）**:
  ...
  ③幾何法線に `vertices[].normal` を使わない。Blender 4.x ではカスタム法線があると
  それがカスタム法線を返し、custom_vs_geo が常に 0°になる（`polygons[].normal` を積む）。
  ④ボーンチェーンから Head 系ボーンを除く。除かないと Head 自身がチェーンルートになり、
  「髪が何にぶら下がっているか」を答えられない。
  ①②は「一見それらしい指標が実は別のものを測っていた」型、
  ③④は「Blender 実機でしか露見しない」型。
  新しい指標を足すときは、**その指標が高くなる別の理由**を先に潰すこと。
  そして **bpy 依存の経路は合成データの自己テストでは一切守られない**（③④は実機で初めて出た）。
- **既知の限界（0.2.0）**: 連結成分＝房の前提（頂点結合済みの髪は 1 島になる。UV 島分割は未実装）/
  ...
  **ミラー判定はワールド軸で鏡映するので、髪オブジェクトが軸に揃っていないと機能しない**
  （5° 回すだけで matched_fraction が 1.0→0.0。実測）/
  FBX は既定設定（`use_mesh_modifiers=True`）だとモディファイアが焼き込まれ**シェイプキーが消える**ので、
  「Mirror が無い＝片側」「シェイプキーが無い＝作者が作らなかった」と読んではいけない（実測）/
