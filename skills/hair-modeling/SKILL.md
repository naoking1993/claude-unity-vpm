...
**ステータス: v0（2026-08-30作成。inspect_hair.py 0.2.0 は合成データの自己テスト 55 項目のみ通過、
Blender実機は未実測）。**
...
- **0.2.0 で追加した集計**（第2段の昇格候補 7 件に対応。詳細は patterns.md の台帳）:
...
- **既知の限界（0.2.0）**: 連結成分＝房の前提（頂点結合済みの髪は 1 島になる。UV 島分割は未実装）/
  ...
  房ピッチとミラーは O(房数²)（`max_pairwise_strands`=2000 超で省略）/
  エンベロープは targets の全頂点で計算するので髪以外を混ぜると狂う /
