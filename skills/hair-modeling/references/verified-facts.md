...
- Blender 4.5.13 LTS（`(4, 5, 13)`）/ 出典: `bpy.app.version_string` / 2026-08-31
- 実行経路: **PyPI の standalone `bpy` モジュール**（`pip install bpy==4.5.13`、cp311 manylinux wheel）。
  GUI でも Blender MCP でもない。Linux コンテナ / CPython 3.11.15 / 同上 / 2026-08-31
- その環境の numpy: 1.26.4（bpy が依存として入れる）/ 同上 / 2026-08-31
- **ユーザーの GUI Blender と実アセットでは未実測。** 下記の実測はすべて
  standalone bpy ＋ 合成シーン（カード 6 枚・頭楕円体・4 ボーン・カスタム法線・Mirror 未適用）による
- **スクリプトの自己テストを回した環境（Blender ではない）**: CPython 3.11.15 / numpy 2.4.6 /
  Linux コンテナ / 出典: `python3 inspect_hair.py --selftest` 実行 / 2026-08-30

## 原則0（初回実測プロトコル）の結果 — 部分実施

| 手順 | 状態 |
|---|---|
| 1. バージョン・実行経路・OS | 済（上記「環境」） |
| 2. `--selftest` と numpy / API 分岐 | 済。下記「API 分岐」 |
| 3. 房数が目視で数えられる髪で突き合わせ | 済（合成カード 6 枚。実アセットでは未実施） |
| 4. FBX で何が残るか | 済。下記「FBX インポートで残るもの / 消えるもの」 |
| 5. アバター別の頭部基準・front_axis | **未実施**（実アバターが要る） |

**v1 への昇格は保留。** 手順 3 は合成メッシュ、手順 5 は未実施で、
ユーザーの GUI Blender と実アセットでは一度も動かしていない。

## API 分岐（Blender 4.5.13 実測）

- `Mesh.corner_normals` は**ある**、`Mesh.calc_normals_split` は**ない**。
  よって 4.1 未満向けの `calc_normals_split()` 経路は 4.5 では通らず、**未実測のまま** /
  出典: `hasattr` の実測 / 2026-08-31
- **`Mesh.vertices[].normal` と `Mesh.vertex_normals[].vector` は、カスタム分割法線があると
  カスタム法線そのものを返す**（`corner_normals` と完全一致）。
  汚染されないのは `polygons[].normal` だけ。実測: カスタム法線を全ループ +Z にすると
  `vertices[0].normal` が `(-0.294,-0.713,0.637)` → `(0,0,1)` に追従し、
  `polygons[0].normal` は `(0,0.998,-0.062)` のまま / 出典: 直接読み出し / 2026-08-31

## 合成シーンでの実測（Blender 4.5.13、inspect_hair 0.3.0）

カード 6 枚（4列×12行、単調テーパ）／頭楕円体 half=(0.1,0.1,0.12)／
ボーン Head→Hair_01→02→03／根元 Head=1.0／Mirror 未適用／カスタム法線あり。

- 島数 6・列 4・行 12・`flat_card` 6 件・`region_guess` は bangs2/side_-X1/side_+X1/back2 で
  **配置の意図と完全一致**。`uv.along=V` `tip_dir=-V`、根元支配 Head(1.0)、毛先支配 Hair_03、
  `root_lock_t`=0.2、Head ウェイトの落ち方 [1.0, 0.864, 0.352, 0.011, 0.0] /
  出典: `run_blender()` 実行 / 2026-08-31
- Mirror 未適用＋`use_evaluated=False` の警告が出ることを確認 / 同上 / 2026-08-31
- **ミラー判定はオブジェクトの回転に弱い**: 完全に左右対称に置いたカードでも、
  Hair オブジェクトを Z 軸まわりに 5° 回すだけで `mirror.matched_fraction` が
  **1.0 → 0.0** になる。ワールド軸で鏡映するため、軸に揃っていないオブジェクトでは
  対称性を検出できない（`mirror_tol_norm`=0.06 は頭幅比 0.012 相当で、
  半径 0.095 の房が 5° 回ると約 0.017 ずれる）/ 同上 / 2026-08-31
...
Blender 4.5.13 で書き出し→読み込みした実測（合成シーン）。**書き出し設定で結果が変わる。**

| 項目 | `use_mesh_modifiers=True`（既定） | `use_mesh_modifiers=False` |
|---|---|---|
| 頂点数（元 288、Mirror 未適用） | **576**（Mirror が形状に焼き込まれる） | 288（Mirror は落ち、片側のまま） |
| シェイプキー（Basis, Test_SK） | **消える** | 残る |
| 頂点グループ / ウェイト | 残る（4 個） | 残る |
| カスタム法線 | 残る | 残る |
| UV | 残る | 残る |
| モディファイア | Armature のみ再構築。Mirror は消える（焼き込み済） | Armature のみ再構築 |

出典: `export_scene.fbx` → `import_scene.fbx` の往復 / 2026-08-31

解析上の含意:
- **モディファイアが無い FBX 品でも、Mirror が既に焼き込まれている可能性がある。**
  その場合 `Mirror 未適用` 警告は出ないが、形状は両側そろっている（房数も 2 倍）。
- **シェイプキーが無い＝作者が作らなかった、ではない。** 既定設定の書き出しで消える。
- 「FBX にはシェイプキーが残る」という一般的知識は、既定設定では**誤り**だった。

## 既知エラーと対処

- **0.1.0 からのバグ（0.3.0 で修正・Blender 実機でのみ露見）**: 幾何法線に
  `Mesh.vertices[].normal` を使っていたが、Blender 4.x ではカスタム分割法線があると
  これがカスタム法線を返す。結果 `geo_radial_mean_deg` と `custom_radial_mean_deg` が
  常に一致し、`custom_vs_geo_mean_deg` が**常に 0.0°**になっていた。
  TEMPLATE.md §7 の「球体転写らしい / 幾何法線のまま / 判定不能」はこの値で判断するので、
  判定そのものが不可能だった。実測: カスタム法線を +Z にすると geo が 4.0°→85.4° と
  一緒に動いた。対処: `polygons[].normal` を面積で重み付けして頂点に積む（`geo_normal_v1`）。
  修正後は geo が 90.9° で固定され、custom だけが 4.0°/85.4° と動く /
  出典: Blender 4.5.13 + 合成シーン / 2026-08-31
- **0.1.0 からのバグ（0.3.0 で修正・Blender 実機でのみ露見）**: `bl_bone_summary` が
  頂点グループ名と一致するボーンを全部チェーンに入れていたため、**Head ボーン自身が
  チェーンルートになり**、`chain_root_parent_mode` が Head の親（None）になった。
  「髪チェーンが何にぶら下がっているか」を答えるための値なのに、
  根元を Head にウェイトする VRChat 髪の標準構成でちょうど無意味になる。
  `max_chain_depth` も Head を数えて 4（正しくは 3）。
  対処: Head 系ボーンをチェーンから除く純関数 `bone_chain_summary`（`bone_chain_v1`）に分離。
  修正後 `parent_mode=Head` / `max_depth=3` / 出典: 同上 / 2026-08-31
- **0.2.1 の記録漏れ（0.3.0 で修正）**: 文書だけ 0.2.1 に上げ、
  `SCRIPT_VERSION` は 0.2.0 のままだった（原則7 の「修正したら SCRIPT_VERSION を上げる」違反）。
  出力サマリーが `v0.2.0` と表示され、verified-facts の版と食い違っていた / 2026-08-31
...
- Blender 4.1 未満の `calc_normals_split()` 経路は、4.5 では `hasattr` が False にならず
  到達しないため**未実測のまま**（4.0 以前の実機が要る）。
...
- 0.3.0: 実測はすべて合成メッシュ（1 房 48 頂点・6 房）。
  実アセット規模（数万〜十数万頂点・数百房）での所要時間と、
  頂点結合済みの髪が 1 島になる問題の頻度は未実測。
- 0.3.0: `head_object` 指定での頭部基準は実測したが、
  `head_vertex_group` と自動探索（Head ウェイト >=0.5 の頂点 bbox）の経路は未実測。
